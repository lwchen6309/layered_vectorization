#!/usr/bin/env bash
set -eo pipefail

ROOT="/home/lwchen/layered_vectorization/LayeredVectorization"
DATASET_DIR="$ROOT/dataset/DIV2K_test10"
OUTPUT_ROOT="$ROOT/div2k_test10_runs"
MAIN_PY="main.py"
BASE_CONFIG="$ROOT/config/base_config.yaml"
PATHS=(16 32 64 128 256)

source /home/lwchen/miniconda3/etc/profile.d/conda.sh
conda activate lv_svg
set -u

cd "$ROOT"
mkdir -p "$OUTPUT_ROOT/generated_configs"

for NPATH in "${PATHS[@]}"; do
  CFG="$OUTPUT_ROOT/generated_configs/config_${NPATH}.yaml"
  python3 - <<PY
from pathlib import Path
src = Path("$BASE_CONFIG")
dst = Path("$CFG")
text = src.read_text()
lines = text.splitlines()
out = []
seen = False
for line in lines:
    if line.startswith('max_path_num_limit:'):
        out.append(f'max_path_num_limit: {int($NPATH)}')
        seen = True
    else:
        out.append(line)
if not seen:
    out.append(f'max_path_num_limit: {int($NPATH)}')
dst.write_text('\n'.join(out) + '\n')
print(dst)
PY

done

find "$DATASET_DIR" -maxdepth 1 -type f \( -iname '*.png' -o -iname '*.jpg' -o -iname '*.jpeg' \) | sort | while read -r IMG_PATH; do
  IMG_NAME="$(basename "$IMG_PATH")"
  STEM="${IMG_NAME%.*}"

  for NPATH in "${PATHS[@]}"; do
    SAVE_NAME="div2k_${STEM}_p${NPATH}"
    RUN_DIR="$OUTPUT_ROOT/$SAVE_NAME"
    FINAL_SVG="$RUN_DIR/final.svg"
    CFG="$OUTPUT_ROOT/generated_configs/config_${NPATH}.yaml"

    echo "============================================================"
    echo "Running LayerVec on $IMG_NAME | path=$NPATH"
    echo "============================================================"

    if [ -f "$FINAL_SVG" ]; then
      echo "[Skip] final.svg exists: $FINAL_SVG"
      continue
    fi

    if [ -d "$RUN_DIR" ]; then
      echo "[Info] Removing incomplete run dir: $RUN_DIR"
      rm -rf "$RUN_DIR"
    fi

    python "$MAIN_PY" \
      --config "$CFG" \
      --target_image "$IMG_PATH" \
      --file_save_name "$SAVE_NAME" \
      --output_root "$OUTPUT_ROOT"
  done
done

echo "ALL_DONE"
